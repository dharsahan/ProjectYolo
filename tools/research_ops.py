...
# Performance: in-memory cache of the research state plus O(1) lookup sets
# for `queue` and `visited`. Disk is the source of truth on first load,
# but subsequent ops mutate the cache and write once. This eliminates the
# O(n^2) "url not in list" scan per enqueue.
...
def _load_state_locked() -> dict:
    """Load state from disk into the cache (caller must hold _state_lock)."""
    global _state_cache, _queue_set, _visited_set
    ...
    _state_cache = state
    _queue_set = set(state["queue"])
    _visited_set = set(state["visited"])
    return state


def _persist_state_locked() -> None:
    """Write the cached state to disk (caller must hold _state_lock)."""
    if _state_cache is None:
        return
    Path(RESEARCH_STATE_FILE).write_text(
        json.dumps(_state_cache, indent=2), encoding="utf-8"
    )
...
def _is_high_signal_link(
    url: str, text: str, topic_tokens: List[str]
) -> tuple[bool, str]:
    parsed = urlparse(url)
...
def research_enqueue_from_crawl_step(
    ...
    try:
        ...
        return (
            "Error: `crawl_step_output` must be valid JSON from `browser_crawl_step`."
        )
...
def research_queue_urls(urls: List[str]) -> str:
    ...
    Performance: uses set-based dedup (O(1) per check) instead of list scans
    (O(n) per check, O(n^2) total for a batch). Persists once at the end.
    """
    try:
        with _state_lock:
            ...
            added = 0
            for url in urls:
                if url in _visited_set or url in _queue_set:
                    continue
        ...
        return f"Added {added} new URLs to the queue. Total in queue: {queue_len}."
...
def research_get_next() -> str:
    ...
    try:
        with _state_lock:
            if not Path(RESEARCH_STATE_FILE).exists() and _state_cache is None:
            ...
            _queue_set.discard(url)
            state["visited"].append(url)
            _visited_set.add(url)
            _persist_state_locked()
        return url
    except Exception as e:
...
def research_store_summary(url: str, summary: str) -> str:
    ...
    try:
        with _state_lock:
            ...
            _persist_state_locked()
        return f"Summary for `{url}` stored. Total sites analyzed: {total}."
    except Exception as e:

...
def learn_experience(user_id: int, task: str, error: str, resolution: str) -> str:
    ...
    try:
        ...
        memory.add(lesson, user_id=str(user_id), metadata={"type": "experience", "status": "resolved"})
        
        audit_log("learn_experience", {"task": task, "error": error}, "success")
        return f"Technical lesson archived for future tasks: '{error}' -> '{resolution}'."
...
def list_experiences(user_id: int) -> str:
    ...
    try:
        ...
        for m in results:
            ...
            text = m if isinstance(m, str) else (m.get('memory') or m.get('text') or str(m))

...
Examples:
  ...
  $ skillfish search git --limit 3      Limit results to 3`)
    ...
    function outputJsonAndExit(exitCode, queryStr, data = {}) {
        const output = {
    ...
    function exitWithError(message, exitCode, queryStr = query) {
        if (jsonMode) {
            addError(message);
    ...
    if (trimmedQuery.length > 200) {
        exitWithError('Search query too long (max 200 characters)', EXIT_CODES.INVALID_ARGS, trimmedQuery);
    }
    // Parse limit option
    const limitStr = options.limit ?? '5';
    const limit = parseInt(limitStr, 10);
    if (isNaN(limit) || limit < 1 || limit > 50) {
        exitWithError('Limit must be a number between 1 and 50', EXIT_CODES.INVALID_ARGS, trimmedQuery);
    }
    ...
    if (!response.success) {
        exitWithError(response.error ?? 'Failed to search registry', EXIT_CODES.NETWORK_ERROR, trimmedQuery);
    }
    // Transform results for JSON output
    const jsonResults = response.results.map((r) => ({
        name: r.name,
        slug: r.slug,
        owner: r.owner,
        github: r.github,
    ...
    console.log();
    console.log(pc.bold(`Found ${response.results.length} skill${response.results.length === 1 ? '' : 's'}:`));
    console.log();
    // Display results

...
import { readProjectManifest, getProjectManifestPath, parseAllEntries, detectCollisions, deriveSkillDirName, } from '../lib/project-manifest.js';
import { fetchDefaultBranch, fetchRecursiveTree, getSkillSha, SKILL_FILENAME, RateLimitError, RepoNotFoundError, NetworkError, GitHubApiError, } from '../lib/github.js';
import { EXIT_CODES } from '../lib/constants.js';
import { isTTY, isInputTTY, batchMap } from '../utils.js';
// === Command Definition ===
  ...
  $ skillfish install --yes        Skip confirmation prompts`)
    ...
    if (!manifest) {
        ...
        if (!existsSync(manifestPath)) {
            exitWithError(`No manifest found at ${displayPath}. Run ${pc.cyan('skillfish bundle')} to generate one from installed skills.`, EXIT_CODES.NOT_FOUND);
        }
        else {
            exitWithError(`Invalid manifest at ${displayPath}. Check the file format.`, EXIT_CODES.INVALID_ARGS);
        }
    }
    ...
    if (collisions.length > 0) {
        ...
        exitWithError(`${collisions.length} collision(s) found. Fix the manifest and try again.`, EXIT_CODES.INVALID_ARGS);
    }
    // Detect agents
    ...
    if (detected.length === 0) {
        ...
        exitWithError(`No agents detected ${locationLabel}. Install Claude Code, Cursor, or another supported agent first.`, EXIT_CODES.GENERAL_ERROR);
    }
    ...
    if (!isInputTTY() || jsonMode || skipPrompts) {
        ...
        if (!jsonMode) {
            p.log.info(`Detected ${pc.cyan(detected.length.toString())} agent${detected.length === 1 ? '' : 's'}: ${detected.map((a) => a.name).join(', ')}`);
        }
    ...
    if (conflicts.length > 0) {
        ...
        exitWithError(`${conflicts.length} conflict(s) found. See above for details.`, EXIT_CODES.INVALID_ARGS);
    }
        ...
        if (!jsonMode) {
            ...
            if (installs.length > 0) {
                ...
                for (const action of installs) {
                    ...
                    const agentInfo = action.targetAgents.length < targetAgents.length
            ...
            if (toRemove.length > 0) {
                ...
                for (const skill of toRemove) {
                    ...
                    console.log(`  ${pc.red('•')} ${skill.name} ${pc.dim(`(no longer in manifest, from ${agentNames})`)}`);
                }
                console.log();
            }
    ...
    const installResults = await batchMap(toInstall, async (action) => {
        ...
        try {
            ...
            const result = await installSkill(entry.owner, entry.repo, skillPath, skillName, actionTargetAgents, {
    ...
    if (installSpinner) {
        installSpinner.stop(`Installed ${pc.cyan(totalCount.toString())} skill${totalCount === 1 ? '' : 's'}`);
    }
    ...
    for (let i = 0; i < installResults.length; i++) {
        ...
        if (result.success) {
            ...
            if (!jsonMode) {
                ...
                const agentSuffix = agentCount < targetAgents.length
    ...
    for (const skill of toRemove) {
        ...
        if (currentManifest) {
            ...
            const currentKeys = new Set(currentEntries.map((e) => buildManifestKey(e.owner, e.repo, e.path)));
            // Skip removal if skill was added back to manifest (match by key)
            if (skill.manifestKey && currentKeys.has(skill.manifestKey)) {
                if (!jsonMode) {
                    console.log(`  ${pc.yellow('○')} ${skill.name} ${pc.dim('kept (added back to manifest)')}`);
                }
                continue;
            }
        }
        if (!jsonMode) {
            const agentNames = skill.installations.map((i) => i.agent.name).join(', ');
            console.log(`  ${pc.red('✗')} ${skill.name} ${pc.dim(`removed from ${agentNames} (no longer in manifest)`)}`);
        }
        ...
        for (const installation of skill.installations) {
            try {
                ...
                if (!jsonMode) {
                    console.log(`    ${pc.red('!')} Failed to remove from ${installation.agent.name}: ${msg}`);
                }
            }
        }
        if (skillRemoved) {
 ...
 */
function scanInstalledSkills(agents, baseDir) {
...
async function selectInstallLocation(projectFlag, globalFlag, jsonMode) {
    ...
    if (!hasProjectManifest && !hasGlobalManifest) {
        ...
        p.log.info(pc.dim(`Run ${pc.cyan('skillfish bundle')} to create a manifest from installed skills.`));

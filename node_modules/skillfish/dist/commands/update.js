...
import { isInputTTY, isTTY, batchMap } from '../utils.js';
// === Command Definition ===
...
Examples:
  ...
  $ skillfish update --yes --json     Update all outdated skills (JSON output)`)
    ...
    const manifestSkills = allTrackedSkills.filter((s) => (s.manifest.source ?? 'manual') === 'manifest');
    const trackedSkills = allTrackedSkills.filter((s) => (s.manifest.source ?? 'manual') !== 'manifest');
    ...
    if (trackedSkills.length === 0) {
        ...
        if (manifestSkills.length > 0 && !jsonMode) {
            console.log();
            p.log.info(pc.dim(`${manifestSkills.length} skill${manifestSkills.length === 1 ? '' : 's'} controlled by manifest - update ${pc.cyan('skillfish.json')} and run ${pc.cyan('skillfish install')} instead.`));
        }
        if (jsonMode) {
    ...
    const updateResults = await batchMap(outdated, async (skill) => {
        try {

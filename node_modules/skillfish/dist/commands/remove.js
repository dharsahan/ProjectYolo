...
Examples:
  ...
  $ skillfish remove my-skill --agent "Claude Code"  Remove from specific agent`)
    ...
    function warnAboutManifestSkills(skills) {
        const manifestSkills = skills.filter((s) => s.isManifestControlled);
        if (manifestSkills.length > 0 && !jsonMode) {
            console.log();
            p.log.warn(pc.yellow(`${manifestSkills.length} skill${manifestSkills.length === 1 ? ' is' : 's are'} managed by skillfish.json:`));
            for (const item of manifestSkills) {
                console.log(`  ${pc.yellow('•')} ${item.skill}`);
            }
            p.log.info(pc.dim(`These will be reinstalled on next ${pc.cyan('skillfish install')}. ` +

...
import { submitSkillsToRegistry } from '../lib/registry.js';
// === Command Definition ===
...
Examples:
  ...
  $ skillfish submit owner/repo -y                     Skip confirmation`)
    ...
    if (repoArg.includes('github.com')) {
        ...
        try {
            ...
            if (url.hostname !== 'github.com' && url.hostname !== 'www.github.com') {
                exitWithError('Only github.com URLs are supported. Use: https://github.com/owner/repo', EXIT_CODES.INVALID_ARGS);
            }
            const pathParts = url.pathname.split('/').filter(Boolean);
            if (pathParts.length < 2) {
                exitWithError('Invalid GitHub URL. Use: https://github.com/owner/repo', EXIT_CODES.INVALID_ARGS);
            }
        ...
        if (parts.length < 2) {
            exitWithError('Invalid format. Use: owner/repo or https://github.com/owner/repo', EXIT_CODES.INVALID_ARGS);
        }
        [owner, repo] = parts;
    }
    ...
    if (jsonOutput.submitted.length > 0) {
        ...
        p.outro(pc.green(`Submitted! ${skillCount} skill${skillCount === 1 ? '' : 's'} will be reviewed.`));
    }
    else {
 ...
 */
async function discoverSkillsInRepo(owner, repo, jsonMode, jsonOutput) {
    let skillDiscovery;
    try {
    ...
    if (spinner) {
        spinner.stop(`Found ${pc.cyan(skills.length.toString())} skill${skills.length === 1 ? '' : 's'} in ${pc.cyan(`${owner}/${repo}`)}`);
    }
    return { skills };
}

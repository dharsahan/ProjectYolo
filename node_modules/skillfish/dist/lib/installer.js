            ...
            if (existsSync(destDir)) {
                ...
                try {
                    ...
                    if (renameErr.code === 'EXDEV') {
                        result.warnings.push(`${skillName}: Cross-filesystem install, backup protection disabled`);
            ...
            try {
                ...
                if (sha && branch) {
                    try {
                        ...
                        result.warnings.push(`${skillName}: Installed but manifest write failed (update tracking disabled): ${msg}`);
                    }
                }
                ...
                if (hasBackup) {
                    try {
                        ...
                        result.warnings.push(`${skillName}: Installed successfully but backup cleanup failed at ${backupDir}`);
                    }
                }
                ...
                if (hasBackup) {
                    try {
                        ...
                        result.warnings.push(`${skillName}: Installation failed for ${agent.name}, restored previous version`);
                    }
                    catch (rollbackErr) {
                        ...
                        result.warnings.push(`${skillName}: CRITICAL - Installation failed and rollback failed for ${agent.name}. ` +

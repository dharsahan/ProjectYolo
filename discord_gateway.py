    ...
    async def on_message(self, message: discord.Message):
        ...
        async with lock:
            ...
            if session.pending_confirmation:
                await message.reply("Pending confirmation exists. Resolve it in Telegram first.")
                return
            ...
            self.session_manager.save(user_id)
            chunks = [response[i : i + 1900] for i in range(0, len(response), 1900)] or ["(No response)"]
            for chunk in chunks:
...
async def run_discord_gateway() -> None:
    ...
    session_manager = SessionManager(timeout_minutes=int(os.getenv("SESSION_TIMEOUT_MINUTES", "60")))
